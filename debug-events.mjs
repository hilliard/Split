...
async function debug() {
  try {
    ...
    const groups = await sql`SELECT id, name, created_by FROM expense_groups WHERE name = 'Dallas Kids'`;
    ...
    // Get group members
    const members = await sql`SELECT gm.user_id, h.email FROM group_members gm JOIN humans h ON gm.user_id = h.id WHERE gm.group_id = ${groupId}`;
    console.log('👥 Group members:', members);
    console.log(`Is johndoe in group? ${members.some(m => m.user_id === johndoeId)}\n`);

    // Get Dallas Trip event
    const events = await sql`SELECT id, title, creator_id, group_id FROM events WHERE title = 'Dallas Trip'`;
    ...
    console.log(`3. johndoe is member of Dallas Kids? ${members.some(m => m.user_id === johndoeId)}`);
    
    if (events[0].creator_id === johndoeId || (events[0].group_id === groupId && members.some(m => m.user_id === johndoeId))) {
