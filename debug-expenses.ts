...
try {
  ...
  const eventExpenses = await db.select().from(expenses).where(eq(expenses.eventId, firstEvent.id));

  console.log(`Found ${eventExpenses.length} expenses:\n`);
  eventExpenses.forEach((exp, i) => {
    console.log(
      `  ${i + 1}. amount=${exp.amount} cents ($${(exp.amount / 100).toFixed(2)}), tip=${exp.tipAmount} cents ($${(exp.tipAmount / 100).toFixed(2)})`
    );
  });
  ...
  console.log(
    `  Total in cents: ${Number(aggData?.totalAmount || 0) + Number(aggData?.totalTip || 0)}`
  );
  console.log(
    `  Total in dollars: $${((Number(aggData?.totalAmount || 0) + Number(aggData?.totalTip || 0)) / 100).toFixed(2)}`
  );
