...
for (const event of recentEvents) {
  ...
  eventExpenses.forEach((exp, i) => {
    const tip = typeof exp.tipAmount === 'string' ? parseFloat(exp.tipAmount) : exp.tipAmount || 0;
    console.log(
      `    ${i + 1}. $${(exp.amount / 100).toFixed(2)} + $${tip.toFixed(2)} tip = $${(exp.amount / 100 + tip).toFixed(2)} - ${exp.description}`
    );
  });
}
