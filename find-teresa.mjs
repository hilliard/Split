...
const events =
  await sql`SELECT id, title, group_id, created_at FROM events WHERE title ILIKE '%happy%' ORDER BY created_at DESC LIMIT 5`;
...
if (events.length > 0) {
  ...
  if (latest.group_id) {
    const expenses =
      await sql`SELECT id, amount, description, created_at FROM expenses WHERE group_id = ${latest.group_id}`;
