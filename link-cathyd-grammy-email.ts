...
async function main() {
  ...
  try {
    ...
    if (grammy.rows.length > 0) {
      ...
      await client.query(`DELETE FROM email_history WHERE human_id = $1`, [grammy_human_id]);

      // Delete customer record for grammyhaynes
      await client.query(`DELETE FROM customers WHERE human_id = $1`, [grammy_human_id]);

      // Delete human record for grammyhaynes
      await client.query(`DELETE FROM humans WHERE id = $1`, [grammy_human_id]);
