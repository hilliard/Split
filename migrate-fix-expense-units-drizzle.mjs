...
async function main() {
  try {
    ...
    verySuspicious.slice(0, 3).forEach((e) => {
      console.log(
        `  • ${e.description?.substring(0, 30) || '(no desc)'} | ${e.amount} cents = $${(e.amount / 100).toFixed(2)}`
      );
    });
    ...
    const tripOzExpenses = suspiciousExpenses.filter(
      (e) => e.createdAt && new Date(e.createdAt).toISOString().includes('2026-04-2')
    );
    ...
    if (tripOzExpenses.length > 0) {
      ...
      console.log(
        `  If stored wrong: ${totalCents / 100} cents = $${(totalCents / 10000).toFixed(2)}`
      );
