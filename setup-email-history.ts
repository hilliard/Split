  ...
  try {
    ...
    for (const { human_id, email } of customerEmails) {
      try {
        ...
        console.error(`❌ Error for ${email}:`, error instanceof Error ? error.message : String(error));
      }
    }
    ...
    try {
      ...
      console.error(`❌ Error creating grammyhaynes:`, error instanceof Error ? error.message : String(error));
    }
