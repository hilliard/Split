...
export const emailHistory = pgTable(
  ...
  (table) => [
    index('email_history_effective_idx').using(
      'btree',
      table.effectiveFrom.asc().nullsLast().op('timestamp_ops'),
      table.effectiveTo.asc().nullsLast().op('timestamp_ops')
    ),
...
export const settlements = pgTable(
  ...
  (table) => [
    index('settlements_created_at_idx').using(
      'btree',
      table.createdAt.asc().nullsLast().op('timestamp_ops')
    ),
    index('settlements_event_idx').using('btree', table.eventId.asc().nullsLast().op('uuid_ops')),
    index('settlements_from_user_idx').using(
      'btree',
      table.fromUserId.asc().nullsLast().op('uuid_ops')
    ),
    index('settlements_status_idx').using('btree', table.status.asc().nullsLast().op('text_ops')),
    index('settlements_to_user_idx').using(
      'btree',
      table.toUserId.asc().nullsLast().op('uuid_ops')
    ),
    foreignKey({
...
export const activities = pgTable(
  ...
  (table) => [
    ...
    index('idx_activities_start_time').using(
      'btree',
...
export const groupMembers = pgTable(
  ...
  (table) => [
    index('group_members_group_user_idx').using(
      'btree',
...
export const pendingGroupInvitations = pgTable(
  ...
  (table) => [
    index('pending_invitations_group_email_idx').using(
      'btree',
      table.groupId.asc().nullsLast().op('text_ops'),
      table.email.asc().nullsLast().op('text_ops')
    ),
    index('pending_invitations_status_idx').using(
      'btree',
...
export const humanSystemRoles = pgTable(
  ...
  (table) => [
    index('idx_human_system_roles_human_id').using(
      'btree',
      table.humanId.asc().nullsLast().op('uuid_ops')
    ),
    index('idx_human_system_roles_role_id').using(
      'btree',
      table.systemRoleId.asc().nullsLast().op('uuid_ops')
    ),
  ]
);
