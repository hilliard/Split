...
export const GET: APIRoute = async ({ cookies }) => {
  try {
    ...
    if (groupIds.length > 0) {
      whereCondition = or(eq(events.creatorId, userId), inArray(events.groupId, groupIds));
    } else {
    ...
    const eventsWithExpenses = await Promise.all(
      allEvents.map(async (event) => {
        ...
        const totalAmountCents = expenseData?.totalExpenseCents
          ? Number(expenseData.totalExpenseCents)
          : 0;
