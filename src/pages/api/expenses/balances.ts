  ...
  try {
    ...
    try {
      ...
      for (const expense of eventExpenses) {
        ...
        const expenseAmount =
          typeof expense.amount === 'number' ? expense.amount : Number(expense.amount);
        const tipAmount =
          typeof expense.tipAmount === 'number'
            ? expense.tipAmount
            : Number(expense.tipAmount || 0);
      ...
      for (const userId of userIds) {
        ...
        balances.push({
          userId,
          name: userMap[userId]
            ? `${userMap[userId].firstName} ${userMap[userId].lastName}`.trim()
            : 'Unknown',
