...
export const registerSchema = z.object({
  ...
  username: z.string().min(3, 'Username must be at least 3 characters').max(255).transform(val => val.toLowerCase()),
  password: z.string().min(8, 'Password must be at least 8 characters'),
});

export const loginSchema = z.object({
  username: z.string().min(3, 'Username is required').max(255).transform(val => val.toLowerCase()),
