...
try {
  ...
  try {
    ...
    const validPayer = await client.query('SELECT id FROM humans LIMIT 1');
  ...
  console.log('────────────────────────────────────────────────────────────');
  try {
    const validEvent = await client.query('SELECT id FROM events LIMIT 1');
  ...
  try {
    ...
    if (validEvent.rows.length === 0 || validPayer.rows.length === 0) {
      ...
      const result = await client.query(
        ...
        [expenseId, eventId, 5000, 'Test valid expense', payerId, 'food']
      );
  ...
  try {
    ...
    const creator = await client.query('SELECT id FROM humans LIMIT 1');
    ...
    if (creator.rows.length === 0) {
      ...
      const expenseCheck = await client.query('SELECT id FROM expenses WHERE id = $1', [
        testExpenseId,
      ]);

      if (expenseCheck.rows.length === 0) {
        console.log(
          '✅ PASSED: Cascading delete worked - expense was deleted when event was deleted\n'
        );
      } else {
